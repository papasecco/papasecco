...
// Ritrova una domanda a partire dalla chiave salvata negli errori.
function domandaDaChiave(k) {
  const [idBanca, idDomanda] = k.split("::");
  const banca = BANCHE.find(b => b.id === idBanca);
  if (!banca) return null;
  return domandeDi(banca).find(q => q.id === idDomanda) || null;
}
...
function disegnaHome() {
  ...
  html += `<div class="box"><h2>A che punto sei</h2><div class="mini">
      ...
      <div><b>${daRipassare}</b><span>domande da recuperare</span></div>
    </div>`;
  if (ultime.length) {
...
$$(".card").forEach(c => c.onclick = () => {
  if (c.dataset.modalita === "errori") { apriErrori(); return; }
  apriSetup(c.dataset.modalita);
});
...
function aggiornaSetup() {
  ...
  const totali = disponibili().length;
  ...
  $("#f-riassunto").textContent = modalitaSetup === "esame"
    ...
    : modalitaSetup === "allenamento"
      ? `Tutte le ${totali} domande in ordine casuale, senza limite di tempo. Dopo ogni risposta vedi subito la soluzione e la spiegazione.`
      : "Solo le domande che hai già sbagliato, con spiegazione immediata.";
...
// Costruisce la sessione a partire da un elenco di domande già scelto.
function creaSessione(banca, qs, opzioni) {
  const conf = confDi(banca);
  const o = Object.assign({ modalita: "allenamento", durataMs: null, pesata: false, mescolaOpz: true, titolo: null }, opzioni);
  sessione = {
    modalita: o.modalita,
    pesata: o.pesata,
    banca: banca.id,
    titolo: o.titolo || `${banca.titolo} — ${ETICHETTA[o.modalita] || o.modalita}`,
    conf,
    durataMs: o.durataMs,
    inizio: Date.now(),
    indice: 0,
    ...
    domande: qs.map(q => {
      const ordine = o.mescolaOpz ? mescola(q.opzioni.map((_, i) => i)) : q.opzioni.map((_, i) => i);
      return { id: q.id, banca: q._banca, materia: q.materia, testo: q.testo,
               spiegazione: q.spiegazione || "", fonte: q.fonte || "",
...
const ETICHETTA = { esame: "simulazione", allenamento: "allenamento libero", errori: "ripasso errori" };
...
$("#f-avvia").onclick = () => {
  ...
  creaSessione(banca, qs, {
    modalita: modalitaSetup,
    pesata: conPesi,
    mescolaOpz: $("#f-mesc-opz").checked,
    durataMs: modalitaSetup === "esame"
      ? Math.max(1, parseInt($("#f-tempo").value, 10) || conf.durataMinuti) * 60000
      : null
  });
};
...
function rispondi(k) {
  ...
  salvaSessione();
  if (inAllenamento()) {
    const q = sessione.domande[i];
    registraEsito(q, k === q.corretta, q.opzioni[k]);
  }
  disegnaDomanda();
}
...
// ---------- CORREZIONE ----------
function registraEsito(q, giusto, rispostaData) {
  const errori = leggi(CHIAVI.errori, {});
  const k = `${q.banca}::${q.id}`;
  const e = errori[k] || { ko: 0, ok2: 0, materia: q.materia, testo: q.testo };
  if (giusto) e.ok2 = (e.ok2 || 0) + 1;
  else {
    e.ko = (e.ko || 0) + 1; e.ok2 = 0;
    if (rispostaData != null) e.sbagliata = rispostaData;   // testo dell'opzione scelta
  }
...
function consegna() {
  ...
  sessione.domande.forEach((q, i) => {
    ...
    if (!inAllenamento()) registraEsito(q, data === q.corretta, data === null ? null : q.opzioni[data]);
  });
...
// ---------- I MIEI ERRORI ----------
let filtroErrori = null;    // materia selezionata, null = tutte
let soloDaRecuperare = false;

// Errori registrati, riagganciati alla domanda della banca dati.
function elencoErrori() {
  const errori = leggi(CHIAVI.errori, {});
  return Object.entries(errori)
    .filter(([, e]) => e.ko > 0)
    .map(([k, e]) => ({ k, e, q: domandaDaChiave(k) }))
    .filter(x => x.q)
    .sort((a, b) => (b.e.ultima || 0) - (a.e.ultima || 0));
}

const daRecuperare = e => (e.ok2 || 0) < 2;

function apriErrori() {
  filtroErrori = null; soloDaRecuperare = false;
  disegnaErrori();
  mostra("v-errori");
}

function erroriFiltrati() {
  return elencoErrori()
    .filter(x => !filtroErrori || x.q.materia === filtroErrori)
    .filter(x => !soloDaRecuperare || daRecuperare(x.e));
}

function disegnaErrori() {
  const tutti = elencoErrori();
  const aperti = tutti.filter(x => daRecuperare(x.e)).length;
  $("#e-azioni").hidden = !tutti.length;
  $("#e-solo-aperti").classList.toggle("attivo", soloDaRecuperare);
  $("#e-solo-aperti").textContent = soloDaRecuperare ? "Mostra tutte" : "Mostra solo quelle da recuperare";

  if (!tutti.length) {
    $("#e-sottotitolo").textContent = "";
    $("#e-materie").innerHTML = "";
    $("#e-elenco").innerHTML = `<div class="vuoto">Nessun errore registrato. Svolgi una simulazione o un allenamento: le domande sbagliate finiscono qui, con la risposta giusta e la spiegazione.</div>`;
    return;
  }

  $("#e-sottotitolo").textContent =
    `${tutti.length} ${plur(tutti.length, "domanda sbagliata", "domande sbagliate")} in tutto, ` +
    `${aperti} ancora da recuperare. Una domanda esce dall'elenco delle aperte dopo due risposte esatte consecutive.`;

  const perMateria = {};
  tutti.forEach(x => perMateria[x.q.materia] = (perMateria[x.q.materia] || 0) + 1);
  $("#e-materie").innerHTML =
    `<button type="button" class="chip" aria-pressed="${!filtroErrori}" data-materia="">Tutte (${tutti.length})</button>` +
    Object.entries(perMateria).sort((a, b) => b[1] - a[1]).map(([m, n]) =>
      `<button type="button" class="chip" aria-pressed="${filtroErrori === m}" data-materia="${esc(m)}">${esc(m)} (${n})</button>`).join("");
  $$("#e-materie .chip").forEach(ch => ch.onclick = () => {
    filtroErrori = ch.dataset.materia || null;
    disegnaErrori();
  });

  const righe = erroriFiltrati();
  $("#e-elenco").innerHTML = righe.length ? righe.map(({ k, e, q }) => {
    const recuperata = !daRecuperare(e);
    return `<div class="rev ${recuperata ? "" : "ko"}">
      <div class="num">${esc(q.materia)} · sbagliata ${e.ko} ${plur(e.ko, "volta", "volte")}${
        e.ultima ? " · ultima volta il " + dataIt(e.ultima) : ""}</div>
      <p class="t">${esc(q.testo)}</p>
      <ul>${q.opzioni.map((t, i) => {
        const giusta = i === q.corretta;
        const tua = e.sbagliata != null && t === e.sbagliata;
        return `<li class="${giusta ? "g" : (tua ? "s" : "")}">${LETTERE[i]}. ${esc(t)}${
          giusta ? " ← risposta corretta" : (tua ? " ← la tua risposta" : "")}</li>`;
      }).join("")}</ul>
      ${q.spiegazione ? `<p class="sp">${esc(q.spiegazione)}</p>` : ""}
      ${q.fonte ? `<p class="sp">Fonte: ${esc(q.fonte)}</p>` : ""}
      <div class="meta">
        <span class="badge ${recuperata ? "ok" : ""}">${recuperata ? "recuperata" : "da recuperare"}</span>
        <button class="ghost" data-rimuovi="${esc(k)}">Rimuovi dall'elenco</button>
      </div>
    </div>`;
  }).join("") : `<div class="vuoto">Nessun errore con questo filtro.</div>`;

  $$("#e-elenco [data-rimuovi]").forEach(b => b.onclick = () => {
    const errori = leggi(CHIAVI.errori, {});
    delete errori[b.dataset.rimuovi];
    scrivi(CHIAVI.errori, errori);
    disegnaErrori(); disegnaHome();
  });
}

$("#e-solo-aperti").onclick = () => { soloDaRecuperare = !soloDaRecuperare; disegnaErrori(); };

$("#e-allena").onclick = () => {
  const righe = erroriFiltrati();
  if (!righe.length) { alert("Non c'è nessun errore da allenare con questo filtro."); return; }
  const qs = mescola(righe.map(x => x.q));
  const banca = bancaDi(qs[0]._banca);
  creaSessione(banca, qs, {
    modalita: "errori",
    titolo: `${banca.titolo} — ripasso errori${filtroErrori ? " · " + filtroErrori : ""}`
  });
};

$("#e-svuota").onclick = () => {
  if (!confirm("Cancellare l'intero elenco degli errori? L'operazione non è reversibile.")) return;
  localStorage.removeItem(CHIAVI.errori);
  disegnaErrori(); disegnaHome();
};
