...
| `assets/amz-sports.png` | Logo di testata **AMZ Sports**, dall'SVG originale, sfondo trasparente, 760×130 px (visualizzato a 250 px). |
| `assets/amz-court-surface.png` | Logo AMZ Court Surface, estratto dal primo PDF. Non usato nel layout attuale: è l'alternativa di testata per una versione a tema verde. |
...
- Palette: antracite `#1A1A1A` per barra titolo e footer, rosso AMZ `#D61920` (campionato dal logo) come accento, fondo `#F1F2F3`. Ogni marchio porta i propri colori, campionati dai file originali: Greenset `#4CDE71`, Solinco `#EC1B21`, Easygrip `#0089CF` per la barretta della card; per l'occhiello si usano le varianti scurite (`#1E9E4A`, `#C8141A`, `#0072AE`) che restano leggibili su bianco.
- Il neutro scuro è stato scelto perché il logo di testata è rosso e i tre marchi hanno colori propri (verde, rosso, azzurro): una cornice cromatica neutra li lascia convivere senza contrasti. Per tornare al tema verde bastano i tre colori della cornice più il logo `amz-court-surface.png` in testata.
